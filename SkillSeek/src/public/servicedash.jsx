...
const Dashboard = () => {
  ...
  const cards = [
    ...
    },
    {
      ...
      action: () => handleNavigate("/add-service"),
    },
    {
      title: "Notifications",
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className={`${isSidebarOpen ? "w-64" : "w-20"} bg-[#1F4A9B] text-white transition-all duration-300 ease-in-out min-h-screen`}>
        ...
        <nav className="mt-10 space-y-4">
          ...
          <button onClick={() => setViewRequests(true)} className="flex items-center space-x-3 px-4 py-3 hover:bg-blue-800 rounded-lg w-full">
            <FaTools className="w-5 h-5" />
            <span className={`${!isSidebarOpen && "hidden"}`}>Requests</span>
          </button>
          <button onClick={() => handleNavigate("/add-service")} className="flex items-center space-x-3 px-4 py-3 hover:bg-blue-800 rounded-lg w-full">
      ...
      <div className="flex-1">
        ...
        <header className="bg-white shadow-lg py-4 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <img src="https://via.placeholder.com/40" alt="Profile" className="w-10 h-10 rounded-full border border-gray-500 cursor-pointer" />
          </div>
        </header>
